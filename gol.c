...
void Program_Init() {
  ...
  program.window = SDL_CreateWindow("Game Of Life", 
      SDL_WINDOWPOS_CENTERED, 0, WindowSize.x,
  ...
  program.renderer = SDL_CreateRenderer(program.window, -1,
  ...
  button.texture = IMG_LoadTexture(program.renderer,
      "GOL_StartButton.png");
...
void Program_MainLoop() {
  ...
  while (program.isRunning) {
    Program_ProcessEvents(&program.event, &program.isRunning,
        &button.src, &button.dst);
    ...
    if (delta >= 50) {
      ...
      SDL_RenderCopy(program.renderer, button.texture,
        &button.src, &button.dst);
